...
from isaacgymenvs.utils.torch_jit_utils import get_euler_xyz, to_torch, get_axis_params, torch_rand_float, quat_rotate, quat_rotate_inverse, get_euler_xyz
...
class Centaur(VecTask):
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        self.cfg["env"]["numObservations"] = 113
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        print(feet_names)
        print("extremity name", extremity_name)
        ...
        # print("knee indices",self.knee_indices[i])
        # print("feet indices",self.feet_indices)
        print("Number of DOFs:", self.gym.get_asset_dof_count(centaur_asset))
        print("Number of rigid bodies:", self.gym.get_asset_rigid_body_count(centaur_asset))

        # print("self.centaur_handles[0]",self.centaur_handles[0])
...
def compute_centaur_observations(root_states,
    ...
    base_euler_tuple = get_euler_xyz(base_quat)
    # 将元组 (roll, pitch, yaw) 堆叠成一个 [N, 3] 的张量
    base_euler = torch.stack(base_euler_tuple, dim=-1)
    # Base Coordinates
...
def compute_centaur_reward(
    ...
    base_pos = root_states[:, :3]    # sync_error = torch.square(actions[:, 0] - actions[:, 3])

    base_height = base_pos[:, 2]

    base_quat = root_states[:, 3:7]
    # NEW
    base_euler = get_euler_xyz(base_quat)
    ...
    default_gravity = torch.tensor([0, 0, -9.81], device=root_states.device)
    projected_gravity = quat_rotate(base_quat, default_gravity)
